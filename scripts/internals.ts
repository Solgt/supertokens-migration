...
export function loadEnvironmentAndVerifyEnvVars() {
    let ST_API_KEY_SOURCE;
    let ST_CONNECTION_URI_SOURCE;
    let ST_API_KEY_TARGET;
    let ST_CONNECTION_URI_TARGET;
    ...
    ST_API_KEY_SOURCE = process.env.AUTH_SUPERTOKENS_API_KEY;
    ST_CONNECTION_URI_SOURCE = process.env.AUTH_SUPERTOKENS_CONNECTION_URI;
    ...
    ST_API_KEY_TARGET = process.env.AUTH_SUPERTOKENS_API_KEY;
    ST_CONNECTION_URI_TARGET = process.env.AUTH_SUPERTOKENS_CONNECTION_URI;

    if (
        !ST_API_KEY_SOURCE ||
        !ST_CONNECTION_URI_SOURCE ||
        !ST_API_KEY_TARGET ||
        !ST_CONNECTION_URI_TARGET
    ) {
        console.error("Missing environment variables");
        process.exit(1);
    }

    if (!ST_CONNECTION_URI_SOURCE.startsWith("https://st-dev")) {
        console.error(
    ...
    if (!ST_CONNECTION_URI_TARGET.startsWith("https://st-prod")) {
        console.error(
    ...
    return {
        ST_API_KEY_DEV: ST_API_KEY_SOURCE,
        ST_CONNECTION_URI_DEV: ST_CONNECTION_URI_SOURCE,
        ST_API_KEY_PROD: ST_API_KEY_TARGET,
        ST_CONNECTION_URI_PROD: ST_CONNECTION_URI_TARGET,
    };
}
